...
switch _mode do {
	...
	case "Init": {
		...
		//hint format ["DISPLAY: %1\n\nCONTROL: %2", _display, _map];
		//_map ctrlMapSetPosition [0.5,0.9,0.1,0.1];
		
		_map ctrlmapanimadd [0,0.1,[500,50,0]];
		ctrlmapanimcommit _map;

		_map ctrlSetPosition [x, y];
	...
	case "Draw": {
		...
		//--- Draw units
		{
			...
			// *******************************************************
			// !!! assignedTeam can return nil when unit is NULL   !!!
			// !!! or when unit is remote controlling another unit !!!
			// *******************************************************
			if (!isNil "_team") then 
			{
				...
				_colorSide = (missionnamespace getvariable "bis_fnc_groupIndicator_sideColors") select (_sideId + 1);
